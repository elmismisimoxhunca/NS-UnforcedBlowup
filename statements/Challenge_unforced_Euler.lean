...
-- Finite-time blow-up for the 3D incompressible Euler equations with NO external force:
-- smooth, compactly supported, divergence-free, finite-energy initial data whose unique classical
-- solution develops unbounded vorticity as t ↑ T. Definitions above are verbatim from
-- tristanbuckmaster/fluid_lean/euler-blowup/Challenge.lean (Apache 2.0); the force is fixed to zero.
open EulerBlowup in
theorem euler_unforced_blowup :
    ∃ (T : ℝ) (_ : 0 < T) (u : ℝ → R3 → R3) (p : ℝ → R3 → ℝ),
      ClassicalEuler T u (fun _ _ => 0) p ∧
      ...
      (∀ T' < T, 0 < T' → InLipschitzClass3 T' u (fun _ _ => 0)) ∧
      (∀ T' < T, 0 < T' → ∀ u', InLipschitzClass3 T' u' (fun _ _ => 0) → u' 0 = u 0 →
